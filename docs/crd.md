# Bootstrap operator using kubebuilder
...
│       └── zz_generated.deepcopy.go                        # autogenerated implementation of the aforementioned runtime object interfaces
├── bin
...
│   ├── default
│   │   ├── kustomization.yaml
│   │   ├── manager_auth_proxy_patch.yaml                   # provides ccess to metrics endpoint is authenticated and authorized using an auth proxy which is deployed as sidecar container in the manager pod.
...
│   │   ├── auth_proxy_service.yaml
│   │   ├── kustomization.yaml
│   │   ├── leader_election_role.yaml                       # used for leader election by controller sa
│   │   ├── leader_election_role_binding.yaml
│   │   ├── namespaceconfig_editor_role.yaml                # cluster role used by controller sa for write perms                    
│   │   ├── namespaceconfig_viewer_role.yaml                # cluster role used by controller sa for read only perms                    
│   │   ├── role.yaml
│   │   ├── role_binding.yaml                               # rolebinding used by controller sa
│   │   └── service_account.yaml
